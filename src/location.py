...
class Location():
    ...
    def regenerate_agents(self, agent_ids):
        ...
        self.model.life.probabilities =\
            np.append(
                self.model.life.probabilities,
                self.model.life.find_probabilities(list(zip(self.model.age_groups[agent_ids], new_locations)))
            )
        self.model.life.values =\
            np.append(self.model.life.values, [LifeState.ALIVE.value] * len(agent_ids)).astype(np.int16)
        ...
        self.location.previous = np.append(self.location.previous, new_locations).astype(np.int16)
    ...
    def readmission_movement(self):
        ...
        # ----- Find readmissions
        move_ids = [key for key, value in self.readmission_date.items() if value == self.model.time]
    ...
    def update_death_probabilities(self, unique_id: int):
        ...
        self.model.life.probabilities[unique_id] = self.model.life.transition_dict[age, location]
