...
class Antibiotics(SingleEvent):
    def __init__(self, model, enum, transition_dict, key_types, antibiotic_risk):
        ...
        self.initiate_values(
            count=len(self.model.population), value=AntibioticsState.OFF.value
        )
    ...
    def initialize_probabilities(self):
        self.probabilities = self.find_probabilities(
            list(zip(self.model.location.location.values, self.model.age_groups))
        )
    ...
    def step(self):
        ...
        locations = self.model.location.location.values[
            self.model.location.moved_agents
        ]
        ...
        use_agents = self.model.unique_ids[
            (living) & (self.values == self.on) & (self.ends < self.day)
        ]
        probabilities = self.probabilities[use_agents]
        selected_agents = probabilities > self.model.rng.rand(len(probabilities))
        if any(selected_agents):
    ...
    for location in model.location.locations.enum:
        for age in AgeGroup:
            ...
            if category == "COMMUNITY":
                d[(location.value, age.value)] = model.params.location["facilities"][
                    "COMMUNITY"
                ]["age"][str(age.value)]
            elif category == "UNC":
                d[(location.value, age.value)] = model.params.location["facilities"][
