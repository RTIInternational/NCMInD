...
class NcLocationModule(Location):
    def __init__(self, model, params):
        ...
        self.facility_transitions =\
            location_dict(self, pd.read_csv(Path(transition_directory, 'location_transitions.csv')))

        # ----- Community Movement
        ct = pd.read_csv(Path(transition_directory, 'community_transitions.csv'))
        ct['Probability'] = ct['Probability'] * self.params['tuning']["community_probability_multiplier"]
        community_dict = dict()
        ...
        self.transitions['UNC'] = pd.read_csv(Path(transition_directory, "unc_to_unc_transitions.csv")).values
        self.transitions['SMALL'] = pd.read_csv(Path(transition_directory, "small_discharge_transitions.csv")).values
        self.transitions['LARGE'] = pd.read_csv(Path(transition_directory, "large_discharge_transitions.csv")).values
        ...
        self.location.probabilities =\
            self.location.find_probabilities(list(zip(self.model.county_codes, self.model.age_groups)))
        self.location.initiate_values(
            count=len(self.model.population),
            value=self.locations.community
        )

        # ----- Update the community movement probability based on the concurrent conditions
        self.location.probabilites =\
        ...
        for unique_id, item in enumerate(self.model.population.Start_Location.astype(int)):
            if item != 0:
                self.assign_los(
                    unique_id=unique_id,
                    new_location=item,
    ...
    def facility_movement(self):
        ...
        move_ids = [key for key, value in self.leave_facility_day.items() if value == self.model.time]
    ...
    def location_update(self, unique_id: int):
        ...
        previous_category = self.locations.int_to_category[self.location.previous[unique_id]]
        if (previous_category == 'NH') and (current_location in self.locations.all_hospitals):
            new_category = 'NH'
            ...
            if self.model.rng.rand() > self.params['tuning']['NH_to_ST_to_NH']:
                ...
                while new_category == 'NH':
                    new_category = random_selection(self.model.rng.rand(), p, self.locations.categories_list)
                    count += 1
                    if count > 20:
        ...
        if new_category != 'COMMUNITY':
            ...
            elif new_category in [self.locations.category_enum.LARGE.name, self.locations.category_enum.SMALL.name]:
                location_id = self.select_st_hospital(county, logrecno, new_category, current_location)
            else:
            ...
            if current_location in self.locations.all_hospitals:
                ...
                if self.model.rng.rand() < self.params['readmission']['rate']:
                    ...
                    self.readmission_date[unique_id] = self.model.time + self.model.rng.randint(2, 31)
    ...
    def assign_los(self, unique_id: int, new_location: int, initialize: bool = False):
        ...
        if location_category == "NH":
            ...
            if location_category == 'UNC':
                los = self.params['facilities'][self.locations.convert_int(new_location, 'ID')]['los']
            else:
            ...
            if (new_location in self.locations.all_hospitals):
                ...
                if location_category == 'LARGE':
                    logit += 0.1867
                if self.model.concurrent_conditions[unique_id] == ConcurrentConditions.YES:
                    logit += 0.8169
    ...
    def select_unc_hospital(self, county: int, location_int: int):
        ...
        if sum(p) > 0:
            ...
            return random_selection(self.model.rng.rand(), p, self.locations.categories['UNC']['ids'])
        ...
        if len(ut_probs) > 0:
            p = ut_probs[self.model.rng.randint(0, ut_probs.shape[0]), ]
            p = normalize_and_create_cdf(p)
            return random_selection(self.model.rng.rand(), p, self.locations.categories['UNC']['ids'])
        # ----- If no options, send to a large UNC hospital TODO:
        return random_selection(self.model.rng.rand(), [.5, 1.0], ['UNC_7', 'UNC_8'])

    def select_st_hospital(self, county: int, logrecno: int, selected_category: str, location_int: int) -> int:
        ...
        if selected_category == self.locations.category_enum.LARGE.name:
            ...
            # If not, try to use logrecno location
                ...
                if new_location == current_location:
                    ...
                    large_hospitals =\
                        fts[fts[:, self.logrec_columns.get_loc('county')] == county]
                    large_hospitals = large_hospitals[:, self.logrec_columns.get_loc('LARGE')]
                    if any(large_hospitals != current_location):
                        new_location = large_hospitals[large_hospitals != current_location][0]
                    else:
                        # --- Select random large_hospital
                        new_location =\
                            self.model.rng.choice([large for large in large_options if large != current_location])
                return new_location
        # --- Send to Small
        else:
            ...
            return self.model.rng.choice(
                [s for s in small_options if s != self.locations.convert_int(location_int, 'ID')])

    def make_nh_los(self):
        nh_los = pd.read_csv(Path(self.model.experiment_dir, "data/input/cLOS_MDS_2016.csv"))
        nh_los = nh_los[(0 < nh_los.los) & (nh_los.los < 2000)].copy()
        nh_los = nh_los.astype(int)
        nh_los.cfreq = nh_los.cfreq.apply(lambda x: int(x * .1))
        a_list = []
        for row in nh_los.iterrows():
            a_list.extend([row[1].los] * int(row[1].cfreq))

        nh_los2 = pd.read_csv(Path(self.model.experiment_dir, "data/input/nh_time_until_leaving.csv"))
...
class NcLocations(Locations):
    ...
    def __init__(self, directory: str):
        ...
        self.category_enum = IntEnum(
            'Facility Categories',
            {k: _ for _, k in enumerate(self.categories)}
        )
        ...
        # ----- Hospitals
        for _, row in pd.read_csv(Path(self.directory, "data/IDs/hospital_ids.csv")).iterrows():
            self.categories[row.Category]['ints'].append(self.number_of_items)
            self.add_value(row, self.category_enum[row.Category].value)

        # ----- Nursing Homes
        for _, row in pd.read_csv(Path(self.directory, "data/IDs/nh_ids.csv")).iterrows():
            self.categories[row.Category]['ints'].append(self.number_of_items)
            self.add_value(row, self.category_enum.NH.value)

        # ----- LTACHs
        for _, row in pd.read_csv(Path(self.directory, "data/IDs/lt_ids.csv")).iterrows():
            self.categories[row.Category]['ints'].append(self.number_of_items)
            self.add_value(row, self.category_enum.LT.value)

        self.all_hospitals =\
            self.categories['UNC']['ints'] + self.categories['LARGE']['ints'] + self.categories['SMALL']['ints']

        self.enum = IntEnum('Locations', {k: v['int'] for k, v in self.facilities.items()})

        for name in ['UNC', 'LARGE', 'SMALL', 'LT', 'NH']:
            self.categories[name]['ids'] = [self.enum(item).name for item in self.categories[name]['ints']]

        self.community = self.category_enum.COMMUNITY.value
        self.int_to_category = dict()
        for location in self.enum:
            self.int_to_category[location.value] = self.facilities[location.name]['category']
...
def location_dict(model, lt):
    ...
    for item in lt.values:
        ...
        # There is only one row for LT - apply to all LTs though
            ...
            ld[(key_0, key_1, key_2)] = create_cdf(item[lt.columns.get_loc('COMMUNITY'):])
    return ld
