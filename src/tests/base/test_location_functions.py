...
def test_locations_class(model):
    ...
    assert (
        len([item for item in locations.categories["UNC"]["ids"] if "UNC_" in item])
        == 10
    )
    ...
    assert (
        sum([len(locations.categories[item]["ids"]) for item in items])
        == locations.number_of_items - 1
    )

    # ----- Ints and Value lengths match
    assert len(locations.categories["UNC"]["ids"]) == len(
        locations.categories["UNC"]["ints"]
    )
    assert len(locations.categories["NH"]["ids"]) == len(
        locations.categories["NH"]["ints"]
    )
...
def test_select_los(model):
    ...
    for i in range(10):
        ...
        m.location.assign_los(
            unique_id=0, new_location=m.location.locations.facilities["UNC_0"]["int"]
        )
        assert 0 < m.location.current_los[0] < 100
        m.location.current_los[0] = 0
        m.location.assign_los(
            unique_id=0, new_location=m.location.locations.facilities["nonUNC_0"]["int"]
        )
        assert 0 < m.location.current_los[0] < 100
        m.location.current_los[0] = 0
        m.location.assign_los(
            unique_id=0, new_location=m.location.locations.facilities["NH_0"]["int"]
        )
...
def test_find_location_transitions(model):
    ...
    transitions = model.location.find_location_transitions(
        county, age, model.location.location.values[0]
    )
    ...
    for a_loc in model.location.locations.facilities:
        assert (
            sum(
                model.location.find_location_transitions(
                    135, 2, model.location.locations.facilities[a_loc]["int"]
                )
            )
            > 0
        )
...
def test_facility_movement(model):
    ...
    assert sum(m.location.location.values == 0) / len(m.population) > 0.75
...
def test_readmission_movement(model):
    ...
    for unique_id in m.unique_ids:
        m.location.location.values[unique_id] = m.location.locations.facilities[
            "COMMUNITY"
        ]["int"]
        m.location.readmission_date[unique_id] = m.time
        m.location.readmission_location[unique_id] = m.location.locations.facilities[
            "UNC_0"
        ]["int"]
...
def test_select_unc_hospital(model):
    ...
    assert (
        m.location.select_unc_hospital(
            m.county_codes[0], m.location.locations.facilities["UNC_0"]["int"]
        )
        == "UNC_9"
    )
...
def test_initialization(model):
    ...
    assert 0 <= model.location.location.probabilities.max() < 0.05
    assert 0 <= model.location.location.probabilities.min() < 0.05
